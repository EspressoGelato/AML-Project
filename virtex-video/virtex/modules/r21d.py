...
class R2Plus1DNet(nn.Module):
    ...
    def __init__(self, layer_sizes, block_type=SpatioTemporalResBlock, with_classifier=False, return_conv=False,
        ...
        #if self.return_conv:
        self.feature_pool = nn.MaxPool3d(kernel_size=(2, 2, 2), stride=(1, 2, 2))  # 9216
...
if __name__ == '__main__':
    r21d = generate_model(model_depth=34, use_features=True) #R2Plus1DNet((1, 1, 1, 1))

    img = torch.randn(1, 3, 16, 112, 112)#.cuda()
    print(img.shape)
    f = r21d(img)
    print(f.shape)
