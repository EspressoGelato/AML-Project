...
ROOT=$PWD
FOLDER=ucf51_pretrained_prob_fintuning
MODEL=r21d
DEPTH=18
...
--model ${MODEL} --model_depth ${DEPTH} --batch_size 16 --n_threads 16 --checkpoint 25 \
--file_type hdf5 --sample_t_stride 1 --n_val_samples=3 \
--train_crop random --val_freq=1 \
--lr_scheduler multistep \
--learning_rate 0.0006 --weight_decay 5e-4 \
--n_epochs=300
--video_pretrained_path ${Pretrained_PATH}
--finetune_video 
